    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            non_tensor_batch_keys_to_pop = [
                key for key in dict.fromkeys(non_tensor_batch_keys_to_pop)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                for key in extra_non_tensor_keys:
                ...
                with _timer("step", timing_raw):
                    ...
                    if "token_level_scores" not in batch.batch:
                        if len(batch.batch.keys()) == 0:
                            print("[WARN] empty batch after rollout; skipping data metrics.")
                        else:
                            reward_tensor, reward_extra_infos_dict = compute_reward(batch, self.reward_fn)
                            batch.batch["token_level_scores"] = reward_tensor
                    ...
                    if "response_mask" not in batch.batch and len(batch.batch.keys()) > 0:
                        batch.batch["response_mask"] = compute_response_mask(batch)
                    if "advantages" not in batch.batch and "token_level_rewards" in batch.batch:
                        batch = compute_advantage(
                            batch,
                            adv_estimator=self.config.algorithm.adv_estimator,
                            gamma=self.config.algorithm.gamma,
                    ...
                    if "global_token_num" not in batch.meta_info and "attention_mask" in batch.batch:
                        batch.meta_info["global_token_num"] = torch.sum(
                            batch.batch["attention_mask"], dim=-1
                        ).tolist()
                    if "global_token_num" in batch.meta_info and "step" in timing_raw:
