...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    traj_uids = np.asarray(batch.non_tensor_batch['traj_uid'])
    unique_traj_uid, inverse_idx = np.unique(traj_uids, return_inverse=True)
    unique_idx = np.zeros_like(unique_traj_uid, dtype=int)
    for idx, group in enumerate(inverse_idx):
        unique_idx[group] = idx
    ...
    def _maybe_compute_stats(key: str):
        if key in batch.non_tensor_batch:
            values = batch.non_tensor_batch[key][unique_idx]
            return {
                f"episode/{key}/mean": float(values.mean()),
                f"episode/{key}/max": float(values.max()),
                f"episode/{key}/min": float(values.min()),
            }
        return {}
    ...
    metrics = {
        ...
        **_maybe_compute_stats("format_reward"),
        **_maybe_compute_stats("answer_reward"),
        **_maybe_compute_stats("total_reward"),
    }
    return metrics
