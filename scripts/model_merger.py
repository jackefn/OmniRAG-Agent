...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        if not self.model_config.architectures:
            raise NotImplementedError("Model config does not specify architectures, please provide --hf_model_config_path")
        ...
        elif architecture_name in {"Qwen2_5OmniModel", "Qwen2_5OmniForConditionalGeneration"}:
    ...
    def save_hf_model_and_tokenizer(self, state_dict: dict[str, torch.Tensor]):
        ...
        if auto_model_class is Qwen2_5OmniForConditionalGeneration:
            print("Detected Qwen2.5 Omni architecture; building model from config via _from_config().")
        ...
        if auto_model_class is Qwen2_5OmniForConditionalGeneration:
            # Config already contains processor/tokenizer; skip redundant save to avoid errors.
            return
